...
function CreditRating({ disabled, currData, percent, saveStepData, setActiveStepId }) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      ratingLetter: currData?.ratingLetter?.fileUrl || '',
    }),
    [currData]
  );
        ...
        <Stack spacing={3}>
          ...
          <Card>
            <Box display="flex" alignItems="center" sx={{ p: 3, pb: 0 }}>
              <Typography fontWeight={600}>Select Credit Rating Agency</Typography>
            </Box>
            <Grid container p={2}>
              {CardData.map((item, i) => (
                <Grid xs="12" md="3">
                  <Card
                    sx={(theme) => ({
                      ...
                      border:
                        selectedId === i
                          ? (theme) => `solid 1px ${alpha(theme.palette.info.main, 0.9)}`
                          : null,
                      backgroundColor:
                        selectedId === i ? (theme) => alpha(theme.palette.info.main, 0.1) : null,
                      '&:hover': {
          ...
          <Card>
            <Box display="flex" alignItems="center" sx={{ p: 3, pb: 0 }}>
              <Box display="flex" alignItems="center" gap={2}>
                <Box>
                  <Typography fontWeight={600}>Application Details</Typography>
                  {/* <Typography variant="body2" color="text.secondary">
