...
function EscrowSetupView({ percent, setActiveStepId, currData, saveStepData }) {
  ...
  const requiredFields = ['verification', 'expected', 'bank', 'location'];
  const values = watch();
  ...
  }, [values.verification, values.expected, values.bank, values.location]);
  // console.log(values);
    ...
    <Container>
      <FormProvider methods={methods} onSubmit={onSubmit}>
        <Alert severity="success" sx={{ mb: 2 }}>
          <strong>Trust Deed Signed.</strong> Axis Bank escrow accounts can now be opened. All
          escrow accounts will operate under the Trustee Control Agreement, and the platform cannot
          debit funds without dual trustee authorization.
        </Alert>
        <Card>
          <Box
            display="flex"
            ...
            <Box>
              ...
              <Box mt={1}>
                <Typography variant="body2" color="text.secondary">
                  Open 4 dedicated Axis Bank escrow accounts — 1 collection escrow per SPV bucket +
                  1 reserve buffer account.
                </Typography>
                <Typography variant="body2" color="text.secondary">
          ...
          <Stack spacing={3} p={{ xs: 3 }}>
            ...
            >
              <RHFSelect name="bank" label="Bank*">
                {bank.map((role) => (
